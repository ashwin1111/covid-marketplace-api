...
admin.post('/AddMarketPlaces', jwtToken, async function (req, res) {
    ...
    async function chaining(result, countUpdateIdArray) {
        let chain = Promise.resolve();
        for (var h = 0; h < result.rowCount; h++) {
            chain = chain.then(() => dbOperation(result, countUpdateIdArray))
                .then(takeRest)
        }
    }
    ...
    mid = 'mpadid' + id;
    const client = await pool().connect();
    await client.query("INSERT INTO market_place_all_details (market_place_id,market_palce_name,market_place_address,time_slot_ids,customer_max_count,active_check,created_at,on_dates,market_license_number) values($1,$2,$3,$4,$5,$6,now(),$7, $8);", [mid, req.body.market_palce_name, req.body.market_place_address, req.body.time_slot_ids, req.body.customer_max_count, req.body.active_check, req.body.dates, req.body.market_licens], async function (err, result) {
        if (err) {
